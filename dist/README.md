...
1. У конфігурації datasource увімкніть **Enable Streaming Support** (експериментальні функції для цього не потрібні).
...
Streaming працює лише для **PI points** у режимі поточного значення. Не застосовується до:

- summary;
- interpolated;
- recorded values;
- expression;
- AF attributes (не PI point).

У цих випадках плагін використовує звичайний HTTP-запит.

Якщо підписник не встигає обробляти оновлення, повідомлення можуть відкидатися після короткої спроби доставки (до 50 ms); у логах з’явиться попередження з `dropped=true`.
